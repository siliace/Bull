...
namespace Bull
{
    namespace prv
    {
        ...
        GlxContext::GlxContext(const std::shared_ptr<GlxContext>& shared, const ImplPtr<WindowImpl>& window, Uint8 bitsPerPixel, const ContextSettings& settings) :
            GlContext(settings),
            m_window(0),
            m_render(0),
            m_config(nullptr),
        ...
        void GlxContext::createSurface(const ImplPtr<WindowImpl>& handler)
        {
